...
class Archivo
{
   ...
   public:
      ...
      bool AbrirT(const ios_base::openmode t);
      ...
      void EscribirTXT(char lin[180]) { obj<<lin; }
      void EscribirTXT(string c) { obj<<c; }
      void TXT_NL() { obj<<endl; } //TXT_NL = texto nueva linea
};

template <class T>
bool Archivo<T>::AbrirT(const ios_base::openmode t)
{
	obj.open(nom,t);
	if (!EstaAbierto())
	{
		cout<<"El archivo "<<nom<<" no se pudo abrir; quizas no existe, esta corrupto o no se pudo cargar en la memoria."<<endl;
		return false;
	}
	return true;
}
